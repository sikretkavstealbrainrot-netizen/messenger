...
let authFlowInProgress = false; // защита от гонки: не даём auto-redirect сработать раньше времени
...
// Важно: игнорируем это событие, пока идёт наш собственный submit-флоу
// (иначе оно срабатывает сразу после createUserWithEmailAndPassword —
// до того, как успеет записаться профиль в Firestore — и обрывает регистрацию).
onAuthStateChanged(auth, (user) => {
  if (user && !authFlowInProgress) window.location.href = "app.html";
});
